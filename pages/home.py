import flet as ft
import threading
from services.local_db import get_local_workouts  # <-- Nuovo import locale

def home_view(page: ft.Page):
    # recupero dati utente
    user_email = page.session.store.get("user_email")
    user_name = page.session.store.get("user_name") 
    user_oid = page.session.store.get("oid")
    ...
    # Inizializzazione dati utente in locale (Nessun server)
    if not cached_weight or not cached_height or not cached_age:
    ...
    def save_stats(e):
        ...
        # aggiornamento cache locale (Niente chiamate ad Azure!)
    ...
    def load_gym_stats():
        ...
        try:
            # Leggiamo dal database locale
            schede = get_local_workouts(user_email)
